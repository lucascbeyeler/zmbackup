...
ZMBKP_REST_ADMIN="admin@"$OSE_INSTALL_DOMAIN                                                                                                   # Zmbackup's Zimbra admin account for mailbox REST auth (a Zimbra login, not $OSE_USER - Zimbra's REST servlet 401s on that)
ZMBKP_REST_ADMIN_PASS=""                                                                                                                       # ZMBKP_REST_ADMIN's own Zimbra password - not derivable, unlike OSE_INSTALL_LDAPPASS; must be entered at install time
